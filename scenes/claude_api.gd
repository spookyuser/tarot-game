...
func generate_client(request_id: String, game_state: Dictionary) -> void:
	if _api_clients_url.is_empty():
		request_failed.emit(request_id, "Clients API URL not configured")
		return
	...
	add_child(http_request)
	http_request.request_completed.connect(
		_on_client_http_completed.bind(request_id, http_request)
	)

	_pending_requests[request_id] = http_request

	_debug_log("POST %s request_id=%s" % [_api_clients_url, request_id], body)
	var err := http_request.request(_api_clients_url, headers, HTTPClient.METHOD_POST, json_body)
	if err != OK:
		_debug_log("Failed to start client request_id=%s error=%d" % [request_id, err], body)
		_cleanup_request(request_id)
		client_request_failed.emit(request_id, "HTTP request failed to start: %d" % err)
